(function (window) {
    var _Typing = (window.Typing) ? window.Typing : (window.Typing = function () {
        this.original = ""; //問題文
    ...
    function toRoman(input) {
        input = input.replace(/[Ａ-Ｚａ-ｚ０-９！”＃＄％＆’（）＝～｜｛｝‘＊＋＿？＜＞、。￥；：」ー「＠＾－　]/g, function (s) {
            switch (s) {
                case "”":
                    return "\"";
                case "’":
        ...
        for (var i = 0; i < input.length; i++) {
            ...
            while (input[i] == "っ") {
                ...
                if (next && !input[i + 1].match(/[あ-お]/) && !input[i + 1].match(/[な-の]/) && (!checkContractedSounds(next)) && (input[i + 1] != "ゎ")) {
                    temp = [];
